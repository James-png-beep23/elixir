defmodule House3Web.ApartmentLive.FormComponent do
  use House3Web, :live_component
  ...
  defp save_apartment(socket, :edit, apartment_params) do
    ...
    case Server.update_apartment(socket.assigns.apartment.id, apartment_params) do
      {:ok, apartment} ->
  ...
  defp save_apartment(socket, :new, apartment_params) do
    # Use GenServer for creation too
    case Server.create_apartment(apartment_params["name"], String.to_integer(apartment_params["number_floors"])) do
      {:ok, apartment} ->
        notify_parent({:saved, apartment})
        {:noreply,
         socket
